...
function getTitleTags($, selector) {
  var title = getSelectorText($, selector) || getExistingTitle($);
  if (!title) return [];
...
function getDescriptionTags($, selector) {
  var title = getSelectorText($, selector) || getExistingDescription($);
  if (!title) return [];
...
function getImageSrc($, image, endUrl) {
  ...
  const selector = image.replace(interpolateRegex, '$1');
  const node = $(selector).eq(0);
...
function getSelectorText($, selector) {
  if (!selector) return;
  return selector.replace(interpolateRegex, (match, g1) => {
    return getText($, g1) || '';
  });
}
